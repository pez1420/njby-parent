        ...
        <form id="listForm" action="list.jhtml" method="get">
            ...
            <div class="row clearfix">
                <div class="col-md-12">
                    <div class="clearfix">
                        <label>
                            <span style="margin-right: 15px;">设备名称</span>
                            <input type="text" class=" form-control input-sm" placeholder="设备名称" name="name"
                                ...
                                <tbody>
	                                [#list page.content as equipment]
		                                <tr>
		                                    ...
		                                    <td>${equipment.createDate?string("yyyy-MM-dd")}</td>
		                                    <td>
		                                        <a href="#" class="btn btn-search btn-view" val="${equipment.id}"><i class="fa fa-search"></i>查看
		                                        </a>
		                                        <a href="edit.jhtml?id=${equipment.id}" class="btn btn-info"><i class="fa fa-pencil-square"></i>编辑
		                                        </a>
		                                    </td>
		                                </tr>
	                                [/#list]
		...
		$(".btn-view").off("click").on("click", function(event){
			...
			$.post(detailUrl, {"id":id}, function(resp, status){
				if (resp != null && resp.type == "success") {
					var equipment = $.parseJSON(resp.content);
